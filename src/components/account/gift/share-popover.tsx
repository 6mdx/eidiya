...
export function ShareGiftPopover({ name, message }: Props) {
    const [isCopied, setIsCopied] = useState(false)
    const handleCopy = async () => {
        try {
            await navigator.clipboard.writeText(`عيدية من ${name} : ${message}`)
            setIsCopied(true)
    ...
    const url = `عيدية من ${name} : ${message}`
    return (
        <Popover>
            ...
            <PopoverContent dir="rtl" className="w-80">
                <div className="grid gap-4">
                    <div className="flex items-center justify-between">
                        <h4 className="font-medium leading-none">شارك عيديتك 🎁</h4>
